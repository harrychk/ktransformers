...
class ConverterBase:
    ...
    def __init__(
        ...
        # For V4 models with MTP layers: offset where MTP layer indices start in the
        # flattened layer list.  e.g. if main layers are 0..42, mtp.0 maps to 43.
        # Set by _find_expert_layers() after parsing the model keys.
        self.mtp_offset = None
    ...
    def _find_expert_layers(self) -> Dict[int, List[int]]:
        ...
        if self.key_format == "v4":
            # V4 inference format:
            #   Main layers : layers.{L}.ffn.experts.{E}.w1.weight
            #   MTP  layers : mtp.{M}.ffn.experts.{E}.w1.weight
            main_layers = defaultdict(set)
            mtp_layers = defaultdict(set)
            ...
            for key in self.tensor_file_map.keys():
                if ".ffn.experts." in key:
                    ...
                    # mtp.{M}.ffn.experts.{E}.w1.weight    → parts: [mtp,    M, ffn, experts, E, w1, weight]
                    if len(parts) >= 5:
                        prefix = parts[0]       # "layers" or "mtp"
                        idx = int(parts[1])     # layer / MTP index
                        expert_idx = int(parts[4])
                        if prefix == "mtp":
                            mtp_layers[idx].add(expert_idx)
                        else:
                            main_layers[idx].add(expert_idx)

            result: Dict[int, List[int]] = {}
            for layer_idx, expert_set in main_layers.items():
                result[layer_idx] = sorted(list(expert_set))

            # MTP layers are shifted after the last main layer so they get unique
            # integer indices that downstream code can handle transparently.
            # e.g. main layers 0..42 → mtp_offset = 43, so mtp.0 → layer 43.
            if mtp_layers:
                self.mtp_offset = max(main_layers.keys()) + 1 if main_layers else 0
                for mtp_idx, expert_set in sorted(mtp_layers.items()):
                    result[self.mtp_offset + mtp_idx] = sorted(list(expert_set))

            print(f"Found {len(result)} layers with MoE experts (V4 format):")
            for layer_idx, experts in sorted(result.items()):
                if self.mtp_offset is not None and layer_idx >= self.mtp_offset:
                    orig_mtp = layer_idx - self.mtp_offset
                    print(f"  MTP layer mtp.{orig_mtp}: {len(experts)} experts (0-{max(experts)})")
                else:
    ...
    def _load_layer_tensors_from_disk(self, layer_idx: int) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dict[str, torch.Tensor]: Dictionary with keys in format:
                '{blk|mtp}.{layer}.ffn_{proj}_exps.{expert}.numa.{numa_idx}.{weight|scale}'
        """
        ...
        for numa_idx in range(self.threadpool_count):
            ...
            for expert_id in range(self.num_experts):
                ...
                for proj_name, proj_key in proj_mappings:
                    ...
                    weight_key = f"{self._layer_output_prefix(layer_idx)}.{proj_key}.{expert_id}.numa.{numa_idx}.weight"
                    scale_key = f"{self._layer_output_prefix(layer_idx)}.{proj_key}.{expert_id}.numa.{numa_idx}.scale"
        ...
        if self.layout == "fused":
            ...
            def _expert_key(layer_idx, expert_id, proj_name, suffix):
                if self.key_format == "v4":
                    ...
                    # Determine whether this is an MTP layer (shifted index) or a main layer
                    if self.mtp_offset is not None and layer_idx >= self.mtp_offset:
                        mtp_idx = layer_idx - self.mtp_offset
                        return f"mtp.{mtp_idx}.ffn.experts.{expert_id}.{v4_proj}.{v4_suffix}"
                    else:
